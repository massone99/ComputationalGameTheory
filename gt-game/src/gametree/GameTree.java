...
public class GameTree implements GameTreeInterface, Serializable {
	...
	public List<GameNode> getAllNodes() {
        ...
        getAllNodesHelper(getRootNode(), allNodes);
        return allNodes;
    }

    private void getAllNodesHelper(GameNode node, List<GameNode> allNodes) {
        if (node == null) {
            return;
        }
        ...
        for (Iterator<GameNode> it = node.getChildren(); it.hasNext(); ) {
            GameNode child = it.next();
            getAllNodesHelper(child, allNodes);
        }
    }
