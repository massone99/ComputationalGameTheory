...
public class Quiz4_2 {
    ...
    public static void main(String[] args) {
        double[] v = readVectorFromFile("gt-game/resources/C4.txt");
        double[] v1 = computeWeightedVotingGameValues(v);
        Quiz4_2 c = new Quiz4_2(v1);
        c.showGame();
    ...
    public static double[] computeWeightedVotingGameValues(double[] input) {
        double totalReward = input[input.length - 1];
        double threshold = input[input.length - 2];
        double[] weights = Arrays.copyOf(input, input.length - 2);
        double[] v = new double[(int) Math.pow(2, weights.length)];
        ...
        for (int i = 0; i < v.length; i++) {
            ...
            v[i] = sumWeights >= threshold ? totalReward : 0;
        }
    ...
    public Map<String, Double> computeShapleyValues() {
        ...
        for (int i = 0; i < nPlayers; i++) {
            ...
            shapleyValues.put(ids[ids.length-1-i], shapleyValue);
        }
