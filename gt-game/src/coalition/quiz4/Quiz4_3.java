...
public class Quiz4_3 {
    ...
    public static void main(String[] args) {
        double[][] m1 = readMatrixFromFile("gt-game/resources/C7.txt");
        double[] v1 = computeWeightedGraphGameValues(m1);
        Quiz4_3 c = new Quiz4_3(v1);
        c.showGame();
    ...
    private static double[] computeWeightedGraphGameValues(double[][] m1) {
        int n = m1.length + 1; // Number of players
        double[] values = new double[(int) Math.pow(2, n)];
        // Compute values for each coalition
        for (int coalitionMask = 0; coalitionMask < values.length; coalitionMask++) {
            double coalitionValue = 0.0;
            // Iterate over all possible pairs of players
            for (int player1 = 1; player1 < n; player1++) {
                for (int player2 = player1 + 1; player2 <= n; player2++) {
                    // Check if both players are in the current coalition
                    if (((coalitionMask >> (player1 - 1)) & 1) == 1 && ((coalitionMask >> (player2 - 1)) & 1) == 1) {
                        // Adjust player indices to correspond with the matrix indices
                        int m1Row = Math.max(player1 - 1, player2 - 1) - 1;
                        int m1Col = Math.min(player1 - 1, player2 - 1);
                        if ((m1Row + 1) != m1Col)
                            coalitionValue += m1[m1Row][m1Col];
                    }
                }
            }
            values[coalitionMask] = coalitionValue;
        }
    ...
    public static ArrayList<Integer> getSet(long v, int nPlayers) {
    ...
    public Map<String, Double> computeShapleyValues() {
        ...
        for (int i = 0; i < nPlayers; i++) {
            ...
            shapleyValue = Math.round(shapleyValue * 100.0) / 100.0; // Round to the second decimal
